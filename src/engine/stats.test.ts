...
import { demoStats, demoTable } from '../test/helpers.ts'
...
describe('createStats — row-level order-2 → order-1 → unigram backoff', () => {
  ...
  it('yields 0 for unknown rows when unigram is absent (demo parity — the interim JSON shape)', () => {
...
describe('interim demo JSON matches the demo STATS values', () => {
