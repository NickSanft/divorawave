...
| `spell()` letter-offset speller with enharmonic bailout | tonal-backed spelling (correct accidentals) |
...
- **Copy** — canonical strings verbatim from the mockup: tagline `progressions condensed from the ether`; `tuning the ether…`; `tap anywhere to tune in`; both error strings; empty-deck hint; `Hover an orb to taste it. Click to keep it.`; MOTION pill labels (`MOTION ✦ ON` / `MOTION · OFF`).
...
5. **Emit** `public/data/synthwave.transitions.json` (~5–30 KB) with provenance `{ rows_used, filter_tier, generated_at, source: "ailsntua/Chordonomicon", license: "CC-BY-NC-4.0" }`.
...
- A11y pass: AA contrast against actual backgrounds per the tokens contrast table (cyan-on-sunset stays forbidden; `--vw-cyan-text`/ink-halo where needed); cream focus rings visible everywhere; hit targets ≥44 px; labels on all controls; decorative text aria-hidden; keyboard-only run of acceptance steps 2+4; screen-reader spot-check of orb labels and status regions.
...
5. **Engine calibration** (automated, against shipped stats): dial ≤40 diatonic · ~50 first spicy entrant ♭II7 · 60 mixed · ≥90 full deep-ether; Conjure loop-shape invariant holds over 1,000 seeded runs.
...
3. **Two demo parser bugs are fixed, not ported** (flagged in the engine read-through): `CM` parsed as C minor (case-insensitive suffix collision), and `parseRoman`'s case-insensitive direct match returning minor `v`/`i` for typed `V`/`I` in minor keys — defeating the harmonic-minor dominant and picardy I. Phase B does a case-sensitive match first. Regression tests pin both.
4. **Slash-chord bass**: tonal parses it and we store `bass` (brief §6.1 has the field), but the POC voicing engine keeps root-position bass (the demo behavior; brief §8.3 "or specified bass" becomes a backlog item). Deck displays the typed symbol faithfully.
...
13. **Per-run bus adapted for smplr**: smplr fixes its output `destination` at construction, so the demo's per-run bus (every note routed through a throwaway gain) becomes a player-owned `pianoOut` gain feeding a per-run routing gain → FX input, recreated each run and faded on stop with the frozen .06 τ / 400 ms constants. Same audible stop behavior, different wiring.
